...
@section('content')
    <div class="container">
        ...
        @endif
        <form class="container-fluid" enctype="multipart/form-data" action="{{ route('issue.update', [$issue]) }}" method="post">
            @method('put')
            @csrf
            <div class="card">
                ...
                <div class="card-body">
                    <div class="form-group row">
                        ...
                        <div class="text-secondary col-10">
                            <input name="title" class="form-control" value="{{ $issue->title }}">
                        </div>
                    </div>
                                ...
                                @foreach($modules as $module)
                                    <optgroup label="{{ $module->name }}">
                                        @foreach($module->subjects as $subject)
                                            <option value="{{ $subject->id }}" @if ($issue->subject_id == $subject->id) selected @endif>{{ $subject->name }}</option>
                                        @endforeach
                                    </optgroup>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="text-secondary col-10">
                            <textarea name="content" class="form-control markdown-editor">{{ $issue->content }}</textarea>
                        </div>
                    </div>
                    <hr class="row">
                            ...
                            @foreach($issue->attachments as $attachment)
                                <div class="col-4">
                                    <div class="alert p-2 alert-info alert-dismissible fade show" role="alert">
                                        <div class="row">
                                            <input type="hidden" name="old_attachments[]" style="display: none" value="{{ $attachment->id }}">
                                            ...
                                            <div class="col-auto pl-1 flex-grow-1">
                                                <p class="m-0 pt-2" style="line-height: 1.2">
                                                    <b>{{$attachment->name}}</b>
                                                    <i class="d-block">{{number_format($attachment->size/1000, 1) }} KB</i>
                                                </p>
